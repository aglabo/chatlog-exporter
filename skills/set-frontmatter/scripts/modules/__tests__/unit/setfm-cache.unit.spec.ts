...
// @(#): ChatlogCache<SetfmCache> を使ったキャッシュ操作パターンのユニットテスト
//       対象: ChatlogCache (set-frontmatter 固有のキャッシュパターン)
//
...
// constants
/** テスト用エントリファイルパス。ChatlogCache がベース名をキーとして使う動作を確認する。 */
 ...
 */
const _makeCache = async (buf: Map<string, string>, cacheRoot: string): Promise<ChatlogCache<SetfmCache>> => {
  const cache = new ChatlogCache<SetfmCache>('fm-cache', cacheRoot, undefined, {
    cache: {
      readTextFile: (path) => {
...
/**
 * `ChatlogCache<SetfmCache>` を使った set-frontmatter 固有キャッシュパターンのテストスイート。
 *
 * Phase 2+3a（type/category の書き込み）と Phase 3b（frontmatter のマージ書き込み）の
 * 動作を検証する。ChatlogCache 自体のユニットテストは別ファイルで実施済みのため、
...
describe('setfm-cache', () => {
   ...
   *
   * `ChatlogCache.write` は上書きのため、Phase 3b では read → spread merge → write の手順が必要。
   */
