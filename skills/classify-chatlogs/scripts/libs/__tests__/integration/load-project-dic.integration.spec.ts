...
describe('loadProjectDic', () => {
  // ─── T-CL-LPDI-01: projects.dic の正常読み込み ─────────────────────────────

  /**
   * T-CL-LPDI-01: フィクスチャの projects.dic（コメント行・misc を含む）を読み込む場合のテスト群。
  ...
  describe('Given: コメント行と misc を含む projects.dic（YAML形式）', () => {
    ...
    describe('When: loadProjectDic(filePath) を呼び出す', () => {
      /** T-CL-LPDI-01: ProjectDicEntry として正しく変換されることを確認する。 */
      describe('Then: T-CL-LPDI-01 - コメント行を除外し misc を末尾に付けて ProjectDicEntry を返す', () => {
        it('T-CL-LPDI-01-01: app1 が含まれる', async () => {
        ...
        it('T-CL-LPDI-01-02: misc が末尾キーに含まれる', async () => {
          const filePath = `${ASSETS_DIR}/projects.dic`;
          const projects = await loadProjectDic(filePath);
        ...
        it('T-CL-LPDI-01-03: # で始まる名前は含まれない', async () => {
          const filePath = `${ASSETS_DIR}/projects.dic`;
          const projects = await loadProjectDic(filePath);
        ...
        it('T-CL-LPDI-01-04: 空文字列名が含まれない', async () => {
          const filePath = `${ASSETS_DIR}/projects.dic`;
          const projects = await loadProjectDic(filePath);
  ...
  // ─── T-CL-LPDI-03: デフォルトパス（YAML形式）の読み込み ──────────────────

  /**
   * T-CL-LPDI-03: 引数なしで loadProjectDic() を呼び出す場合のテスト群。
  ...
  describe('Given: デフォルトパス .config/chatlog-exporter/projects.dic（YAML形式）', () => {
    ...
    describe('When: loadProjectDic() を引数なしで呼び出す', () => {
      /** T-CL-LPDI-03: トップレベルキーのみが ProjectDicEntry に含まれることを確認する。 */
      describe('Then: T-CL-LPDI-03 - YAMLトップレベルキーのみを返す', () => {
        it('T-CL-LPDI-03-01: 非空辞書が返される', async () => {
          const projects = await loadProjectDic();

          assertEquals(Object.keys(projects).length > 0, true);
        });

        it('T-CL-LPDI-03-02: misc が末尾キーに含まれる', async () => {
          const projects = await loadProjectDic();
        ...
        it('T-CL-LPDI-03-03: 空文字列名が含まれない', async () => {
          const projects = await loadProjectDic();
        ...
        it('T-CL-LPDI-03-04: aplys が含まれる（YAMLトップレベルキーとして認識される）', async () => {
          const projects = await loadProjectDic();

          assertEquals('aplys' in projects, true);
        });

        it('T-CL-LPDI-03-05: rules を name とするエントリが含まれない（ネストキーは除外される）', async () => {
          const projects = await loadProjectDic();

          assertEquals('rules' in projects, false);
        });

        it('T-CL-LPDI-03-06: when を name とするエントリが含まれない（ネストキーは除外される）', async () => {
          const projects = await loadProjectDic();
  ...
  // ─── T-CL-LPDI-02: ファイルなし → ChatlogError('FileDirNotFound') をスロー ────

  /**
   * T-CL-LPDI-02: 存在しないファイルパスを渡した場合のテスト群。
  ...
  describe('Given: 存在しないファイルパス', () => {
    ...
    describe('When: loadProjectDic("/nonexistent/path") を呼び出す', () => {
      /** T-CL-LPDI-02: ファイル不在時に ChatlogError がスローされることを確認する。 */
      describe('Then: T-CL-LPDI-02 - FileDirNotFound エラーがスローされる', () => {
        it('T-CL-LPDI-02-01: ChatlogError(FileDirNotFound) がスローされる', async () => {
          await assertRejects(
