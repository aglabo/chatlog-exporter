...
description: >
  ...
  ディレクトリに、_scripts/（共有ライブラリ）をスキル自身の隣に配置し、
...
共有ライブラリ `_scripts/` だけは基準が違い、**このスキル自身の隣**（スキルが置かれている
skills ディレクトリ直下）に配置される。各スキルは `../../_scripts/` で共有ライブラリを import
...
共有ライブラリの実体 `skills/_scripts/` そのものになる。
...
（どちらも「skills ディレクトリにスキルが同居し、`_scripts` がその兄弟」という同じ形になる）。
...
| `_scripts/`                        | スキルの隣（skills 直下） | `_scripts/`                 |
...
- `_scripts/` — 各スキルが共有する型・定数・ライブラリ。各スキルの `../../_scripts/` import が
  ...
  インストールスコープに応じて `~/.claude/skills/_scripts/`（User スコープ）または
  `<project>/.claude/skills/_scripts/`（プロジェクトスコープ）になる

展開先は既存ファイルを個別にマージせず、エントリ単位で扱う。ローカルの編集を保つため、
既存の展開先はスキップがデフォルトになっている。

**スキル更新時の注意:** `_scripts/` も既存ならスキップされるため、スキルを新しい版に
