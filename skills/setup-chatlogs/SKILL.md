...
**1. ソースリポジトリのルートでの実行はスクリプトが拒否する。**
chatlog-exporter では `.claude/skills` が `../skills` への symlink になっているため、展開先
`.claude/skills/_scripts` は symlink 経由で共有ライブラリの実体 `skills/_scripts/` に解決する。
そのまま展開すると配布物に含まれない `__tests__/` などを削除してしまうため、スクリプトは
展開先がスキルのソースツリーに重なることを検出した時点で、**何も配置せずエラーで停止する**。
これは `--force` の有無にかかわらず、`rm -rf` を実行する前に判定される。
...
この経路はソースツリーに重ならないため 1. の拒否は働かない。実行前にカレントディレクトリを
必ず確認すること。
