...
**1. ソースリポジトリのルートで `--force` を実行してはならない。**
chatlog-exporter では `.claude/skills` が `../skills` への symlink になっているため、`--force` は
展開先 `.claude/skills/_scripts` を symlink 経由で解決し、共有ライブラリの実体である
`skills/_scripts/` を削除してしまう。同様に `deno.json` もコピー元とコピー先が同一になる。
...
引数なしの実行であれば展開先が既存のエントリはスキップされるため、ソースリポジトリのルートでは
無害だが、そもそも各展開先に実体が既に存在しており実行する意味がない。
