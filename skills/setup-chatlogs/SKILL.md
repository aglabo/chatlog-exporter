...
カレントディレクトリ基準にすると、User スコープ（`~/.claude/skills/`）にインストールされた
スキルからは import が解決できず、実行時にエラーになる。
...
共有ライブラリがスキルの隣に置かれる以上、chatlog-exporter のチェックアウトでは展開先が
共有ライブラリの実体 `skills/_cle-libs/` そのものになる。
`--force` でそのまま展開すると、この実体が配布物のコピーで置き換わり、
配布物に含まれない `skills/_cle-libs/__tests__/` などが失われる（同居する他のスキルは消えない）。
...
`.config/chatlog-exporter/` と `deno.json` はカレントディレクトリ基準のため、
リポジトリのサブディレクトリで実行してもエラーにならず、その場所に作られてしまう。
