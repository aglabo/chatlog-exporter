 ...
 *
 * ## 別ツリーを `--input-dir` で処理した場合も同じ経路をたどる
 *
 * キーがファイル名である以上、陳腐化は再 export に限らない。`strip-cache` は対象ツリーのパスを
 * キーに含めず、agent / period ごとにも分かれていないため、異なるツリーに同名のファイルがあると
 * 前のツリーの記録が引かれて R-003 が `done` を返す。`--input-dir` で任意のツリーを指定できる
 * ようになったため、コピーやエクスポートで作った別ツリーを処理する場合にこれが起こりうる。
 *
 * 防御は 2 段構えである。同一実行の列挙内で衝突するベース名は `strip-chatlogs` の受理ゲート
 * （`DuplicateBasename`）が判定・書き込み・キャッシュ記録より前に弾く。実行をまたぐ衝突は
 * 上記と同じくキャッシュの手動削除で解く。内容ハッシュや mtime による自動無効化を採らない理由も
 * 上記と同じである。
 *
