...
describe('generateOutputFileName', () => {
  ...
  describe('When: 正常系', () => {
    it('[Normal] T-NC-SIO-06-01-01: index=0 のとき hashFn 戻り値を使い <baseName>-01-<hash7>.md を返す', async () => {
    ...
    it('[Normal] T-NC-SIO-06-01-02: index=1 のとき連番が "02" になる', async () => {
      const filePath = 'chatlogs/claude/2026/2026-03/test-file.md';
    ...
    it('[Normal] T-NC-SIO-06-01-03: index=9 のとき連番が "10" になる', async () => {
      const filePath = 'chatlogs/claude/2026/2026-03/test-file.md';
    ...
    it('[Normal] T-NC-SIO-06-04-01: ソースの末尾ハッシュを除去したベース名で出力名を生成する', async () => {
 ...
 * 正常系・エッジケースを検証する。
 *
 * テスト ID 範囲: T-NC-SIO-11-01-01 〜 T-NC-SIO-11-02-01
 *
...
describe('generateSegmentFile', () => {
  ...
  describe('When: 正常系', () => {
    it('[Normal] T-NC-SIO-11-01-01: 返却文字列に `## Summary\\nFix CI pipeline` が含まれる', () => {
    ...
    it('[Normal] T-NC-SIO-11-01-02: 返却文字列に START_BODY_HEADING + "\\n### User\\nHow do I..." が含まれる', () => {
  ...
  describe('When: エッジケース', () => {
    it('[Edge] T-NC-SIO-11-02-01: 返却文字列に `## Summary` と START_BODY_HEADING の両セクション見出しが含まれる', () => {
...
describe('attachFrontmatter', () => {
  ...
  describe('When: 正常系', () => {
    ...
    it('[Normal] T-NC-SIO-12-01-01: 出力フロントマターに project: "ci-platform" が含まれる', () => {
    ...
    it('[Normal] T-NC-SIO-12-01-02: 出力フロントマターに title・log_id が含まれ summary は含まれない', () => {
      fm.set('project', 'ci-platform');
      const segmentMeta = { title: 'Fix CI', log_id: 'abc1234' };
      const content = '## Summary\nFix CI';
    ...
    it('[Normal] T-NC-SIO-12-03-01: 出力が `---\\n` で始まりフロントマターブロックが `\\n---\\n` で終わる', () => {
      fm.set('project', 'test');
    ...
    it('[Normal] T-NC-SIO-12-03-02: コンテンツボディがフロントマターブロックの後に重複なく続く', () => {
      const segmentMeta = { title: 'T', log_id: 'x' };
      const content = '## Summary\ntext';
  ...
  describe('When: エッジケース', () => {
    ...
    it('[Edge] T-NC-SIO-12-02-01: 出力フロントマターが AI 生成フィールド（title・log_id）のみを含む', () => {
 ...
 * 出力のみを行う。
 *
 * テスト ID 範囲: T-NC-SIO-WS-01 〜 T-NC-SIO-WS-07
 *
...
describe('writeSegmentToFile', () => {
  ...
  /** 正常系: ファイル書き込みを検証する。 */
  describe('When: 正常系', () => {
    it('[Normal] T-NC-SIO-WS-01: outputFileName のファイルが書き込まれる', async () => {
      // act
    ...
    it('[Normal] T-NC-SIO-WS-02: 既存ファイルがある場合 .old-01.md にバックアップされ新ファイルが書かれる', async () => {
      // arrange — 既存ファイルを事前作成
      const expectedFile = `${outputDir}/sample-01-testhash.md`;
    ...
    it('[Normal] T-NC-SIO-WS-04: 返されたパスが元の filePath とは異なる（入力ファイルを上書きしていない）', async () => {
      // act
  ...
  describe('When: 異常系', () => {
    it('[Error] T-NC-SIO-WS-05: outputPath が filePath を含むとき ChatlogError(ForbiddenOutput) をスローする', async () => {
  ...
  describe('When: エッジケース', () => {
    it('[Edge] T-NC-SIO-WS-06: old-98.md が存在するとき old-99.md にバックアップして成功する', async () => {
    ...
    it('[Error] T-NC-SIO-WS-07: old-99.md が存在するとき TooManyBackups エラーをスローする', async () => {
