 ...
 *
 * テスト ID 範囲: T-CLS-CE-01 〜 T-CLS-CE-08
 *
 * @see ChatlogError
 */
describe('ChatlogError', () => {
  ...
  describe('constructor', () => {
    ...
    describe('When: 正常系', () => {
      it('[Normal] T-CLS-CE-01: kind=InvalidFormat, subindex=NotClosed, detail=msg → message === Invalid Format: msg', () => {
        const _err = new ChatlogError('InvalidFormat', 'NotClosed', 'msg');
        assertEquals(_err.message, 'Invalid Format: msg');
      });

      it('[Normal] T-CLS-CE-02: kind と subindex プロパティが正しく設定される', () => {
        const _err = new ChatlogError('InvalidFormat', 'NotClosed', 'msg');
        assertEquals(_err.kind, 'InvalidFormat');
        assertEquals(_err.subindex, 'NotClosed');
      });

      it('[Normal] T-CLS-CE-03: name === ChatlogError', () => {
        const _err = new ChatlogError('InvalidFormat', 'NotClosed', 'msg');
        assertEquals(_err.name, 'ChatlogError');
      });

      it('[Normal] T-CLS-CE-04: instanceof Error が true', () => {
        const _err = new ChatlogError('InvalidFormat', 'NotClosed', 'msg');
        assertInstanceOf(_err, Error);
      });

      it('[Normal] T-CLS-CE-05: instanceof ChatlogError が true', () => {
        const _err = new ChatlogError('InvalidFormat', 'NotClosed', 'msg');
        assertInstanceOf(_err, ChatlogError);
      });

      it('[Normal] T-CLS-CE-06: kind=FileDirNotFound → message === File Or Dir Not Found: detail', () => {
    ...
    describe('When: 異常系', () => {
      it('[Error] T-CLS-CE-07: detail 省略時に message === Invalid Format: Undefined Error', () => {
    ...
    describe('When: エッジケース', () => {
      it("[Edge] T-CLS-CE-08: detail=''（空文字）のとき message === Invalid Format: ", () => {
        const _err = new ChatlogError('InvalidFormat', 'NotClosed', '');
