 ...
 *
 * ## 判定ルール
 * - `decision === 'DISCARD'` かつ `confidence >= DISCARD_THRESHOLD` → ファイルを削除（dryRun=false 時）
 * - `confidence < DISCARD_THRESHOLD` → DISCARD 判定でも KEEP 扱い
...
describe('processChunk', () => {
  ...
  /**
   * DISCARD 判定だが `confidence` が `DISCARD_THRESHOLD`（0.7）未満の前提条件グループ。
   *
