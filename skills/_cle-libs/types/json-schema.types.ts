 ...
 *
 * `firstField` は §4.3.1 の契約定義表が規範として持つ「復元の起点」列を表す。
 * 意味を持つのは `yaml` 契約だけであり、契約タグによる判別共用体として型で必須にしている。
 *
 * - `json-array`（#1・#2・#3）: 起点は envelope `items` であり、envelope 名は契約タグ側が定める
 * - `line-prefixed`（#6）: 起点は行頭 `type:` / `category:` のような required keys そのもので、
 *   単一の起点キーが存在しない（§4.3 が「キー集合は契約定義と完全一致」と定める）
 * - `yaml`（#4・#5）: 起点は firstField `title` / `validity` であり、同じタグでも呼び出し元ごとに
 *   異なる。契約タグからは導けないため契約定義が持つ
 *
 * `yaml` で `firstField` を欠いた契約定義は型として構築できない。ただし型が防げるのは
 * 起点キーの**不在**だけであり、`properties` に無いキーを指す**誤り**は防げない。
 * `firstField ∈ keys(properties)` の検査は `validateOutputContract` と `restoreContractText` が
 * 実行時に行い、破れていれば `ChatlogError('AiError', 'ResponseSchemaViolation')` を投げる。
 *
 * 各メンバの契約タグは {@link OutputContractTag} から導出する。リテラルを展開して書くと
 * タグ型が参照ゼロになり、タグを追加してもどこにも型エラーが出ない（復元側の
