...
//       対象: buildJsonSchema / assertOutputContractValues / shouldBuildJsonSchema
//
...
// ─── Test target
import { assertOutputContractValues, buildJsonSchema, shouldBuildJsonSchema } from '../../json-schema-builder.ts';
...
/**
 * 値域検証が設定エラーとして失敗することを検証し、その message を返す。
 *
 * 分類は `kind === 'AiError'` / `subindex === 'ResponseSchemaViolation'` に固定される
 * （根拠は T-LIB-AI-JSB-04-04 のコメント / DR-18 決定 1 / DR-26）。
 *
 * @param contract - 値域規則に違反する出力契約
 * @returns 送出された `ChatlogError` の message
 */
const _assertValuesViolation = (contract: OutputContract): string => {
  const _error = assertThrows(() => assertOutputContractValues(contract), ChatlogError);
...
/**
 * structured-output 仕様 §4.3.1 #5（`setfm-review.ts`）の契約定義に、辞書由来 enum を付与したもの。
 *
 * 入れ子 object `corrected_frontmatter` の `type` / `category` は単一値 enum でフォールバック値を持ち、
 * `topics` / `tags` は配列要素の enum でフォールバック値を持たない（§4.3.1「配列値の enum」）。
 */
const SETFM_REVIEW_ENUM_CONTRACT: OutputContract = {
  contract: 'yaml',
  firstField: 'validity',
  properties: {
    validity: { type: 'string' },
...
/**
 * T-LIB-AI-JSB-10 で検証する実契約相当のケース。いずれも値域規則に適合し、throw しない。
 *
 * ID は規約 §4-1 によりケースごとに分ける（#1→10-01 … #6→10-06）。
 */
const _VALID_CONTRACT_CASES: { id: string; label: string; contract: OutputContract }[] = [
  { id: 'T-LIB-AI-JSB-10-01', label: '#1 classify（project fallback misc）', contract: CLASSIFY_ENUM_CONTRACT },
  { id: 'T-LIB-AI-JSB-10-02', label: '#2 filter（decision fallback ERROR）', contract: PROCESS_CHUNK_CONTRACT },
  { id: 'T-LIB-AI-JSB-10-03', label: '#3 segment-ai（enum なし・配列要素 object）', contract: SEGMENT_AI_CONTRACT },
  {
    id: 'T-LIB-AI-JSB-10-04',
    label: '#4 setfm-frontmatter（配列要素 enum）',
    contract: SETFM_FRONTMATTER_ENUM_CONTRACT,
  },
  { id: 'T-LIB-AI-JSB-10-05', label: '#5 setfm-review（入れ子 object の enum）', contract: SETFM_REVIEW_ENUM_CONTRACT },
  {
    id: 'T-LIB-AI-JSB-10-06',
    label: '#6 setfm-type-category（単一値 enum）',
    contract: SETFM_TYPE_CATEGORY_ENUM_CONTRACT,
  },
];

/**
 * T-LIB-AI-JSB-11-06 で使う、同一キー `type` に置く 3 理由の単一値 enum。
 *
 * 順に 空値域 / 空文字列を含む値域 / 値域外のフォールバック値。path が同じため、
 * message の差は理由の文言だけから生じる。
 */
const _THREE_REASON_TYPE_SPECS: OutputFieldSpec[] = [
  { type: 'string', values: [], fallback: 'x' },
  { type: 'string', values: ['', 'x'], fallback: 'x' },
  { type: 'string', values: ['x'], fallback: 'y' },
];

/** T-LIB-AI-JSB-12-02 / 12-03 で使う、配列要素 enum `topics` の値域が空（空辞書）の契約。 */
const _EMPTY_TOPICS_CONTRACT: OutputContract = {
  contract: 'yaml',
  firstField: 'title',
  properties: {
    title: { type: 'string' },
 ...
 *
 * テスト ID 範囲: T-LIB-AI-JSB-01-01 〜 T-LIB-AI-JSB-06-01（T-LIB-AI-JSB-04-05 を含む）
 *
 * @see buildJsonSchema
 */
describe('buildJsonSchema', () => {
  ...
  describe('辞書由来 enum', () => {
    ...
    it('[Error] T-LIB-AI-JSB-04-04: 値域に存在しないフォールバック値は設定エラーとして失敗する', () => {
      ...
      // 落ちる経路を作らない。`InvalidArgs` は `isAbortingAiError` / `isFatalAiError` の
      // いずれにも該当せず、DR-18 が DR-12 を supersede した当の述語ギャップを再現する。
      // subindex は §3.2（llama 経路の分類一覧の唯一の所有者）の 6 分類から選び、新設しない
      // （DR-26「既存の分類が当てはまるなら subindex を増やさない」）。契約定義自身の不変条件
      // 違反を続行側の `ResponseSchemaViolation` に固定した `output-contract.ts`
      // `_assertFirstField` と同じ扱いに揃える。
      assertEquals(_error.kind, 'AiError');
      assertEquals(_error.subindex, 'ResponseSchemaViolation');
    });

    it('[Error] T-LIB-AI-JSB-04-05: 配列要素 enum の値域に空文字列を含む契約は、値域検証を経由して同分類で失敗する', () => {
...
/**
 * `assertOutputContractValues` のユニットテストスイート。
 *
 * 契約定義の全 enum（トップレベル・入れ子 object・配列要素・配列要素 object）を走査し、
 * 単一値 enum の値域が空・値域が空文字列を含む・単一値 enum のフォールバック値が値域外、のいずれかを
 * 設定エラーとして拒否し、配列要素 enum の空値域（空辞書）は許容することを検証する（§4.3.1「辞書に無い値を暗黙に補ってはならない」）。
 *
 * テスト ID 範囲: T-LIB-AI-JSB-10-01 〜 T-LIB-AI-JSB-12-03
 *
 * @see assertOutputContractValues
 */
describe('assertOutputContractValues', () => {
  /** 実契約相当の値域規則に適合した契約で throw しないケース。 */
  describe('When: 正常系', () => {
    for (const { id, label, contract } of _VALID_CONTRACT_CASES) {
      it(`[Normal] ${id}: ${label} の値域検証が throw せず undefined を返す`, () => {
        assertEquals(assertOutputContractValues(contract), undefined);
      });
    }
  });

  /** 値域違反を設定エラー（AiError / ResponseSchemaViolation）として拒否するケース。 */
  describe('When: 異常系', () => {
    it('[Error] T-LIB-AI-JSB-11-01: トップレベル単一値 enum のフォールバック値が値域外なら path と値を示して throw する', () => {
      const _message = _assertValuesViolation(INVALID_FALLBACK_CONTRACT);

      assertStringIncludes(_message, 'type: ');
      assertStringIncludes(_message, 'gamma');
      assertStringIncludes(_message, 'フォールバック値');
    });

    it('[Error] T-LIB-AI-JSB-11-02: 入れ子 object の単一値 enum のフォールバック値が値域外なら `corrected_frontmatter.type` を示して throw する', () => {
      const _contract: OutputContract = {
        contract: 'yaml',
        firstField: 'validity',
        properties: {
      ...
      assertStringIncludes(_message, 'corrected_frontmatter.type: ');
      assertStringIncludes(_message, 'implementation');
    });

    it('[Error] T-LIB-AI-JSB-11-03: 配列要素 enum の値域に空文字列を含むなら `tags[]` を示して throw する', () => {
      const _contract: OutputContract = {
        contract: 'yaml',
        firstField: 'title',
        properties: {
      ...
      assertStringIncludes(_message, 'tags[]: ');
      assertStringIncludes(_message, '空文字列');
    });

    it('[Error] T-LIB-AI-JSB-11-04: 単一値 enum の値域が空なら、フォールバック理由より先に空値域として throw する', () => {
      const _contract: OutputContract = {
        contract: 'line-prefixed',
      ...
      assertStringIncludes(_message, 'type: ');
      assertStringIncludes(_message, '値域が空');
      assertEquals(_message.includes('フォールバック値'), false);
    });

    it('[Error] T-LIB-AI-JSB-11-05: 配列要素 object のプロパティの違反を envelope を含まない `segments[].kind` で示して throw する', () => {
      const _contract: OutputContract = {
        contract: 'json-array',
        properties: {
          segments: {
      ...
      // envelope `items` は契約定義に現れないため path の先頭に付かない
      assertStringIncludes(_message, ' segments[].kind: ');
      assertStringIncludes(_message, '"z"');
    });
  ...
  /** 空辞書由来の値域（`['']` / `[]`）の扱い。`['']` は拒否し、配列要素 enum の `[]` は許容する。 */
  describe('When: エッジケース', () => {
    it("[Edge] T-LIB-AI-JSB-12-01: 単一値 enum の値域が空辞書由来の [''] のみなら `category` を示して空文字列として throw する", () => {
      const _contract: OutputContract = {
        contract: 'line-prefixed',
      ...
      assertStringIncludes(_message, 'category: ');
      assertStringIncludes(_message, '空文字列');
      // 値域を JSON 表記で示し、空文字列を `[]`（空値域）と見分けられるようにする
      assertStringIncludes(_message, '[""]');
    });

    it('[Edge] T-LIB-AI-JSB-12-02: フォールバック値を持たない配列要素 enum の値域が空（空辞書）なら throw しない', () => {
      assertEquals(assertOutputContractValues(_EMPTY_TOPICS_CONTRACT), undefined);
    });

    it('[Edge] T-LIB-AI-JSB-12-03: 配列要素 enum の値域が空なら buildJsonSchema が enum: [] の配列ノードを返す', () => {
      const _schema = buildJsonSchema(_EMPTY_TOPICS_CONTRACT);

      // 数量制約を足さない（R-002）。空配列だけが「該当なし」として許容される
      assertEquals(_schema.properties.topics, { type: 'array', items: { type: 'string', enum: [] } });
    });
  });
});
