// src: skills/_cle-libs/libs/__tests__/file-io/unit/read-utils.unit.spec.ts
 ...
 * throw せず `Error` インスタンスを返す（ファイル I/O 起因ではないエラーは throwFileIoError の値に関わらず re-throw する）。
 *
...
describe('readTextFile', () => {
   ...
   *
   * 例外を投げず ChatlogError(FileDirNotFound) が返ることを検証する。
   */
  describe('Given: 存在しないファイルパスを渡し throwFileIoError: false を指定する', () => {
    /** readTextFile を実行するとき。 */
    describe('When: readTextFile を実行する', () => {
      /** 例外を投げず ChatlogError(FileDirNotFound) が返ることを検証する。 */
      describe('Then: T-LIB-U-RF-08 - 例外を投げず ChatlogError(FileDirNotFound) が返る', () => {
        it('T-LIB-U-RF-08: NotFound は例外を投げず ChatlogError(FileDirNotFound) を返す', async () => {
          const _result = await readTextFile('/nonexistent/path.md', {
            readProvider: _notFoundProvider,
            throwFileIoError: false,
          });
          assertEquals(_result instanceof ChatlogError, true);
          assertEquals((_result as ChatlogError).kind, 'FileDirNotFound');
          assertEquals((_result as ChatlogError).subindex, 'NotFound');
        });
      });
    });
  });
   ...
   *
   * 例外を投げず Error インスタンスが返ることを検証する。
   */
  describe('Given: 読み込み権限のないファイルを渡し throwFileIoError: false を指定する', () => {
    /** readTextFile を実行するとき。 */
    describe('When: readTextFile を実行する', () => {
      /** 例外を投げず Error インスタンスが返ることを検証する。 */
      describe('Then: T-LIB-U-RF-09 - 例外を投げず Error が返る', () => {
        it('T-LIB-U-RF-09: PermissionDenied は例外を投げず Error を返す', async () => {
          const _result = await readTextFile('/restricted/path.md', {
            readProvider: _permissionDeniedProvider,
            throwFileIoError: false,
          });
          assertEquals(_result instanceof Deno.errors.PermissionDenied, true);
          assertEquals((_result as Error).message, 'permission denied');
        });
      });
    });
  });
   ...
   *
   * 中間パスコンポーネントがファイルであるケースを想定し、例外を投げず Error インスタンスが返ることを検証する。
   */
  describe('Given: NotADirectory エラーが発生し throwFileIoError: false を指定する', () => {
    /** readTextFile を実行するとき。 */
    describe('When: readTextFile を実行する', () => {
      /** 例外を投げず Error インスタンスが返ることを検証する。 */
      describe('Then: T-LIB-U-RF-11 - 例外を投げず Error が返る', () => {
        it('T-LIB-U-RF-11: NotADirectory は例外を投げず Error を返す', async () => {
          const _result = await readTextFile('/tmp/file/child.md', {
            readProvider: _notADirProvider,
            throwFileIoError: false,
          });
          assertEquals(_result instanceof Deno.errors.NotADirectory, true);
          assertEquals((_result as Error).message, 'not a directory');
        });
      });
    });
  });
