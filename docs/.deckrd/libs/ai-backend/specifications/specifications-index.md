...
現時点で、複数ファイルに跨る未決事項はない。

解決済みの未決事項を次に示す。

| 旧番号           | Question                                                                                             | 解決                                                                                                                                                                                                            |
| ---------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1                | 対象サーバが `response_format` を準拠 / 黙殺 / 拒否のどれで扱うか                                    | 準拠（honoured）。3 スキーマ × 3 条件の 9 組すべてで 10/10（`measurements-response-format-2026-09-12.md` / structured-output §4.1.1）。対応対象は測定した 1 構成に限る（DR-31）                                 |
| 2（v1.3.0 時点） | `response_format` の拒否（中断）とコンテキスト長超過（続行）が同じ HTTP 400 で返る場合の読み分け手段 | `error.message` の接頭辞 `JSON schema conversion failed` で判別する（DR-33 / error-handling §4.1）。追加実測（`measurements-response-format-rejection-2026-09-15.md`）で不正なスキーマの 400 を観測して確定した |
...
| 2026-09-12 | 1.3.0   | Phase 0 実測の結果を反映 (MINOR: 実装対象を確定させる決定)。§4 の未決 #1 (`response_format` の実対応レベル) を解決済みへ移し、着地が準拠 (honoured) であることを記録。未決 #2 (HTTP 400 の読み分け) は実測で 400 が発生しなかったため既定の維持として記述を改めた                                                                                                         |
| 2026-09-15 | 1.4.0   | DR-33 を反映 (MINOR: 実装対象を確定させる決定)。§4 の未決 #2 (HTTP 400 の読み分け) を解決済みへ移した（旧番号は v1.3.0 時点の #2）。未決の表は空になった                                                                                                                                                                                                                  |
