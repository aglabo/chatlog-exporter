...
| - | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| 2 | `response_format` の拒否（中断）とコンテキスト長超過（続行）が同じ HTTP 400 で返る場合の読み分け手段が未定である。実測では HTTP 400 が 1 件も発生しなかったため（`measurements-response-format-2026-09-12.md` §3.2）、判別条件は実測から決められない。判別できない 400 を続行側の `ExitFailure` に落とす既定を維持する | error-handling（transport の R-006 / R-008 の分類にも波及） |
