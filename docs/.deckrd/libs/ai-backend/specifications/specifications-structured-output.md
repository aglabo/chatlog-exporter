...
| R-004   |    6 | 共有の配列パーサの **直接パース段**（入力全体、またはコードフェンス内を取り出したテキスト）が呼び出され、対象テキストが構文的に有効な空配列である | 例外を投げず、空配列を成功結果として返す。後続の括弧マッチ段（間接抽出段）は空配列を受理せず、パース失敗として次段へ送る。空配列受理を段 1 に限る理由と、フェンス除去経路にも適用する決定は DR-28 が持つ                                                                                         |
...
### 4.1.1 実測結果の記録（REQ-F-016 の帰結）

R-006 の実測は完了した。記録は `measurements-response-format-2026-09-12.md` が唯一の正であり、
本節はその結論のみを持つ。**着地は「準拠（honoured）」である。**
ただし、**「スキーマ強制が効かないまま通常運転を続ける llama 経路」は、いずれの分岐でも
許容しない**。これは REQ-F-003 が「構造化出力を強制する」ことを目的としており、強制なしの
運転はその目的を満たさないためとする。

| 実測結果                 | 実測の結論                                                                          | 実行時の扱い                                                                                                       |
| ------------------------ | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| 準拠（honoured）         | **本構成の着地点**。3 スキーマ × 3 条件の 9 組すべてで 10/10（DR-25 / DR-31）       | llama 経路を通常どおり有効にする                                                                                   |
| 黙殺（silently ignored） | 観測されなかった。`finish_reason` は 90 回すべて `stop`                             | 当該サーバ実装を対応対象外とする。実行時には R-008 が個々の応答を不適合として `ResponseSchemaViolation` に分類する |
| 拒否（rejected）         | 観測されなかった。HTTP は 90 回すべて 200 であり、400 は 1 件も発生していない（注） | HTTP エラーとして扱う。`response_format` の拒否と判別できた場合は `ResponseFormatRejected`（中断側）               |

（注）妥当なスキーマに対する拒否は観測されていない。不正なスキーマに対しては 400 が返ることを
追加実測で確認した（`measurements-response-format-rejection-2026-09-15.md`）。これはサーバ実装の拒否ではなく個々のスキーマの拒否であり、
着地（準拠）は変わらない。判別条件は DR-33 / `specifications-error-handling.md` §4.1 が持つ。

黙殺・拒否が実測されたサーバ実装は対応対象外とする。スキーマ強制なしの degraded 運転は
選択肢として提供しない（REQUIREMENTS §1.2 Out of Scope / REQ-F-016）。

対応対象は測定レポート §1 の 1 構成に限る（DR-31）。thinking を無効化する起動オプションを
欠いた構成、量子化レベルの異なるモデル、他のサーバ実装は未実測であり対応対象外とする。
Section 5 の「サーバが `response_format` を無視する / 拒否する」行は、実測では観測されなかった
経路の実行時規定として引き続き有効とする。
...
**合格線**（DR-25）: 試行回数は組ごとに 10 回とし、測定した組すべてで 10/10（100%）を合格とする。
1 組でも 10/10 に満たない場合、当該サーバ実装・当該モデル条件を対応対象外とする。

**実測した範囲**（DR-31 / `measurements-response-format-2026-09-12.md`）: モデル差の条件は
測定しなかった。対象サーバに量子化レベルの異なる 2 種目のモデルがロードされていないためであり、
対応対象を測定レポート §1 の 1 構成に限定することで代えた。残る 3 条件 × 3 スキーマの 9 組は
すべて 10/10 で合格している。対応対象を広げるときは、当該構成について 3 スキーマ × 10 試行を
測り、測定レポートへ追記する。
...
| 応答のコードフェンス内が空配列 `"[]"` で、フェンス外に非空の配列が続く                           | 直接パース段がフェンス内を取り出して空配列を成功として返す。括弧マッチ段によるフェンス外の配列の救済へは進まない（R-004 / DR-28）                                                                                    | REQ-F-013 | フェンス内の空配列はモデルの妥当な出力形であり、「最初にパースできたものが勝つ」を優先規則とする。段ごとに優先規則を分けると同一パーサ内で挙動が分岐する                                                               |
...
適用先は直接パース段に限る。括弧マッチ段は従来どおり空配列を受理しないため、
散文中の無関係な括弧対を配列と誤認する経路は開かない（DR-28）。
...
**判定結果**: classify と normalize は **適合**、filter は **不適合** とする。

classify は突合が該当なしを返してフォールバックへ収束する。キャッシュ書き込みも突合結果が
空であることに帰着するため差が出ない。normalize は空配列を受け取ると全ファイルを未分割のまま
返すが、これはパース失敗時の戻り値と一致する。差は診断ログのみであり（パース失敗時の 1 件から、
ファイルごとに「応答に該当項目がない」旨を出す形へ変わる）、後者のほうが実態に即している。
空配列時の戻り値を検証する既存テストも維持される。

filter（`skills/filter-chatlogs/scripts/modules/filter/process-chunk.ts`）は観測点 1（戻り値）と
観測点 3（`stats`）の双方が変わるため不適合とする。パース失敗時はチャンク内の全件を `stats.error` へ
加算し `ChatlogError('InvalidFormat', 'JsonParse')` を返すが、空配列を受理すると突合ループへ進み、
全 entry が判定不能となって `stats.skip` が加算され、戻り値は `undefined` になる。

この差は上表の「呼び出し元ごとに意図した結果かを判定する」に従って **意図した結果** と判定する。
チャンク全件を error として扱う旧挙動は、モデルが正当に「該当なし」を返した場合を失敗と
誤判定するものであり、R-004 が解消しようとした誤判定そのものにあたる。旧挙動を固定する既存テストも
存在しない。したがって REQ-C-002 に対する意図的な例外として本節に記録し、非破壊条件の対象外とする。

<!-- impl-note: 呼び出し元 3 箇所（classify / filter / normalize）は parsed が truthy になるため
     パース失敗分岐に落ちなくなる。T-01 実装時の実測で、classify はフォールバックへ収束し、
     normalize は `_nullMap()` により戻り値がパース失敗時と一致することを確認した。
     filter のみ戻り値と `stats` の内訳が変わる（上記の不適合判定を参照）。 -->
...
<!-- Q1（`response_format` の実対応レベル）は解決済み。
     着地は準拠（honoured）。根拠: `measurements-response-format-2026-09-12.md` / Section 4.1.1。
     対応対象は測定レポート §1 の 1 構成に限る（DR-31）。
...
| 2026-09-15 | 2.4.1   | §4.1.1 の「拒否」行に注記を追加 (PATCH: 明確化)。不正なスキーマに対する 400 を追加実測で確認したことと、判別条件の所在（DR-33 / error-handling §4.1）を記録。着地（準拠）は変更なし                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
