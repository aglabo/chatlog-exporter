...
どの段まで空配列を受理するかは DR-06 が定めていません。実装
（`skills/_cle-libs/libs/text/json-utils.ts`）は次の構造を持ちます。

1. 段 1 `_parseDirectArray` は、入力が `[` で始まる場合は入力全体を、そうでない場合は
   コードフェンス内を取り出したうえで `_tryParseArray` に渡します
2. 段 2 `_parseFirstBracketMatch` は non-greedy な括弧マッチ、段 3 `_parseGreedyBracketMatch` は
   greedy な括弧マッチで、いずれも抽出した区間をパースします

段 1 の 2 経路のうち、素の経路は入力全体の `JSON.parse` 成功を要求するため、空配列を受理しても
後続段を潰しません（`T-LIB-J-15`: `[] [{"a":1}]` は段 1 が失敗し、段 2 が `[{"a":1}]` を救済します）。
一方フェンス経路は最初のフェンスの外側を捨てるため、フェンス内が `[]` でフェンス外に実配列がある
入力では、空配列を受理すると段 1 が `[]` を返して短絡します（`T-LIB-J-22-03`）。
つまりフェンス経路に空配列受理を広げるかどうかは、DR-06 の文言からは決まらず、
独立した決定を要します。

**Decision**:

1. 段 1 のフェンス除去経路にも空配列受理を適用する。素の経路とフェンス経路で `allowEmpty` の
   値を分けない
...
- フェンス経路だけ `allowEmpty=false` に据え置く — フェンス内の `[]` はローカルモデルが
  「該当なし」を返すときの妥当な出力形であり、これを失敗として扱うと段 2 が拾った
  無関係な配列を返してしまいます。DR-06 が解消しようとした誤判定をフェンス付き応答に対して
  残すことになるため不採用
- フェンス除去後にフェンス外の配列の有無を確認し、実配列があればそちらを優先する —
  「最初にパースできたものが勝つ」を捨てることになり、先頭の素配列が後方のフェンス例示に
  勝つ既存の挙動（`T-LIB-J-19`）と方針が分岐します。同じパーサの中に 2 つの優先規則を
  抱えることになるため不採用
- 全段で空配列を受理する — 散文中の偶発的な括弧対を空配列と誤認する経路が開きます
