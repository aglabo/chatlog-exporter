...
| ------------------------------------------------------------ | --------- | ----- | ----------- |
| T-01: `judgeType` 返り値を void 化                           | 3         | 7     | in-progress |
| T-02: `judgeCategory` 引数・返り値を簡略化                   | 3         | 6     | done        |
| T-03: `generateFrontmatter` を entry 直接書き込みに変更      | 3         | 7     | done        |
| T-04: `reviewFrontmatter` を entry 直接書き込みに変更        | 3         | 6     | done        |
| T-05: `writeFrontmatter` から `FrontmatterResult` 依存を除去 | 2         | 4     | done        |
...
- `skills/set-frontmatter/scripts/modules/__tests__/unit/judge-type.unit.spec.ts`（シグネチャ追従）
- `skills/set-frontmatter/scripts/modules/__tests__/integration/judge-pipeline.integration.spec.ts`（judgeType ブロックのみ変更、他フェーズは後続タスクで変更）
- `skills/set-frontmatter/scripts/__tests__/fixtures/frontmatter.fixtures.spec.ts`（シグネチャ追従）
...
- [x] **T-01-01-01**: `judgeType` を呼び出した後、各 entry の `frontmatter.get('type')` が有効な type 値を返す
...
- [x] **T-01-01-02**: オーケストレーターで `typeMap` が不要になる（`typeChunks.flat()` / `typeMap.get()` が削除される）
...
- [x] **T-01-02-01**: AI が無効な type を返したとき `DEFAULT_FALLBACK_TYPE` が entry に書き込まれる
...
- [x] **T-01-02-02**: AI 呼び出しが例外を throw したとき `DEFAULT_FALLBACK_TYPE` が entry に書き込まれる
...
- [x] **T-01-03-01**: `prompts.prompts.get('type')` が `undefined` のとき `ChatlogError` を throw する
...
- [x] **T-01-04-01**: `frontmatter.fixtures.spec.ts` の `judgeType` 呼び出し後の検証が、
...
- [x] **T-02-01-01**: `judgeCategory` 呼び出し後、`entry.frontmatter.get('category')` が有効な category 値を返す
...
- [x] **T-02-01-02**: `type` 引数が削除され、オーケストレーターが `categoryMap` を使わなくなる
  - Target: `set-frontmatter.ts`
...
- [x] **T-02-02-01**: AI が無効な category を返したとき `DEFAULT_FALLBACK_CATEGORY` が entry に書き込まれる
...
- [x] **T-02-03-01**: `prompts.prompts.get('category')` が `undefined` のとき `ChatlogError` を throw する
...
- [x] **T-02-04-01**: `frontmatter.fixtures.spec.ts` の `judgeCategory` 呼び出しが
      `type` 引数なしになり、返り値参照が `entry.frontmatter.get('category')` に変わる
  - Target: `frontmatter.fixtures.spec.ts`
  - Scenario: Given シグネチャ変更後, When fixtures テストを実行する
  - Expected: Then 全件 GREEN

- [x] **T-02-04-02**: オーケストレーターの `type` フォールバック（`?? 'research'`）が削除され、
...
- [x] **T-03-01-01**: `generateFrontmatter` 呼び出し後、`entry.frontmatter.get('title')` が非空文字列になる
...
- [x] **T-03-01-02**: `type` / `category` 引数が削除され、`entry.frontmatter.get()` から取得する
...
- [x] **T-03-01-03**: `fmResultMap` が削除され、`entry` が直接 Phase 3.5 に渡される
  - Target: `set-frontmatter.ts`
...
- [x] **T-03-02-01**: `runAI` が throw したとき entry の既存フィールドは変更されない
...
- [x] **T-03-03-01**: `cleanYaml` が空文字列を返したとき、entry の frontmatter は変更されない
...
- [x] **T-03-04-01**: `frontmatter.fixtures.spec.ts` の `generateFrontmatter` 呼び出しが
      `type`/`category` 引数なしになり、返り値参照が `entry.frontmatter.get()` に変わる
  - Target: `frontmatter.fixtures.spec.ts`
  - Scenario: Given シグネチャ変更後, When fixtures テストを実行する
  - Expected: Then 全件 GREEN

- [x] **T-03-04-02**: `generateFrontmatter` の返り値が `boolean` となり、
      オーケストレーターが `_generatedFiles: Set<string>` で成功ファイルを管理する
  - Target: `setfm-frontmatter.ts`, `set-frontmatter.ts`
  - Scenario: Given T-03 完了後
  - Expected: Then `fmResultMap` が `_generatedFiles` に置き換えられている
...
- [x] **T-04-01-01**: `validity === 'fail'` のとき `entry.frontmatter.get('type')` が修正値に更新される
...
- [x] **T-04-01-02**: `validity === 'pass'` のとき entry の frontmatter は変更されない
...
- [x] **T-04-02-01**: Phase 3.5 完了後の Phase 4 への受け渡しが `entries[]` のみになる
  - Target: `set-frontmatter.ts`
...
- [x] **T-04-03-01**: `correctedType` が空文字列のとき `entry.frontmatter.get('type')` は変更されない
...
- [x] **T-04-03-02**: `runAI` が throw したとき entry は変更されない
...
- [x] **T-04-04-01**: `ReviewResult` から `correctedType`/`correctedCategory`/`correctedYaml` フィールドが削除される
...
- [x] **T-05-01-01**: `writeFrontmatter(entry, dryRun, stats)` で動作する
...
- [x] **T-05-01-02**: yaml 空チェックが entry ベースに変わる
...
- [x] **T-05-02-01**: `setfm-write.unit.spec.ts` の `_makeResult()` ヘルパーと `writeFrontmatter` 呼び出しが削除または変更される
  - Target: `setfm-write.unit.spec.ts`
  - Scenario: Given シグネチャ変更後, When ユニットテストを実行する
  - Expected: Then 全件 GREEN

- [x] **T-05-02-02**: `FrontmatterResult` 型が `phase.types.ts` から削除できる
  - Target: `phase.types.ts`
