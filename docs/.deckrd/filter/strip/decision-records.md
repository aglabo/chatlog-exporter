...
テキストが本体として設置され、成功として報告され、R-010 の一括削除で新しい内容が失われます。
...
- Negative:
  - **前提が破れた場合、新しい内容は黙って失われます。** 検出も報告もされず、R-010 が `.bak` を
  ...
  - `write-utils.ts` の `AlreadyExists` 復旧経路 (既存を削除してから rename を再試行する。
    失敗すると原文も新内容も残らない) は本 DR の射程外であり、引き続き残ります
