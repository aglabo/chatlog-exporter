...
複数 `## Summary` を持つファイルは 976 件ありますが、先頭 strip 後も定型部が残るのは 2 件のみです。
Option A で 6398 件を処理でき、取りこぼしは 6 件（残存 2 + 後方配置 4）＝ 0.09% にとどまります。
...
- Negative:
  - 処理中はディスク使用量が実質倍になる（対象 6402 件・元データ 275.3MB 規模）
...
実測では対象 6402 件に対し passthrough が 3888 件あるため、
...
「6402 件を除去した」実行と「全件が処理済みで 1 件も除去しなかった」実行が
サマリー上で区別できません。
...
キャッシュに処理済みの記録を持つファイルは、本体を開かずに passthrough (処理済みスキップ) とします。
...
- Negative:
  ...
  - 先頭 strip 後もなお定型部が残るファイル (実測 2 件) は、キャッシュ喪失後の再実行で
    再度 strip されます
    → `.bak` が存在する間は R-004 が防ぎます。`.bak` 削除後かつキャッシュ喪失後に限られます。
    実測 11671 件中 2 件 (0.02%) であり、Q-02 が 6 件の取りこぼしを許容した先例に倣い許容します
