...
ファイルが差し替わる経路は残ります。本 DR のガードもこの窓を狭めるだけで閉じません
(Consequences 参照) 。
...
- Negative:
  ...
  - **検証はスワップ地点ではなく `splice` の直前に置くため、競合の窓は狭まりますが閉じません。**
    `writeTextFile` は一時ファイルへ書き出した後に退避コールバックを呼ぶため、本ガードと
    `backupToBak` のリネームの間には `Deno.writeTextFile` の I/O が丸ごと入ります。
    この間に差し替えが起きると、新しい内容が `.bak` へ退避され、古い内容から作った strip 済み
    テキストが本体として設置されたうえで成功として報告されます。R-010 が当該 `.bak` を削除するため
    新しい内容は失われます
...
**Phase**: impl-fix
**Status**: Accepted
**Supersedes**: DR-07 (入力ディレクトリ拒否の部分)

### Context

DR-07 は、REQ-C-004 の前提が claude/2026-07 の実測に基づくことを根拠に、strip の受理範囲を
`<agent> <YYYY-MM>` に限定し、`--input-dir` による override を拒否しました。
6000 件規模の破壊的書き換えが未検証のデータセットへ及ぶことを防ぐ安全弁です。

しかし実運用のログは `classify-chatlogs` によりプロジェクト別サブディレクトリへ移動しており、
`<agent> <YYYY-MM>` 形で解決される `originalLogs/<agent>/<年>/<YYYY-MM>/` 直下の `.md` は 0 件です
(実測: claude/2026-07 直下 0 件・配下 338 件) 。加えて `normalizeLogs/` 配下のログには
`addOnDir` が `originalLogs` に固定されているため到達する手段がありません。

利用者は、他スキル (`filter` / `noise-filter` / `classify`) と同じく `--input-dir` で
対象ディレクトリを指定できる形を求めました。

### Decision

1. **`--input-dir` および第 1 位置引数のパスを受理します。** 共通 `parseArgs` のパターン A が
   `inputDir` へ格納した値を `resolveChatlogsDir()` の `override` へ渡します
2. **入力ディレクトリ指定時は年月を要求しません。** `override` 指定時は agent / period / addOnDir が
   対象の解決に使われないため、年月を必須とする根拠が成立しません
3. **出力ディレクトリの拒否は維持します** (DR-32) 。strip は in-place で書き換えるため
   出力先の概念を持たず、指定された値は対象の解決に用いられません
4. **未検証データセットへの適用の可否は利用者の判断に委ねます。** 破壊的書き換えに対する
   安全側の担保は、退避 (`.bak`) と R-011 の保持ゲート (error が 1 件でもあれば退避を全保持) が
   引き続き担います
...
| Option | 内容                                          | 判定                                                                         |
| ------ | --------------------------------------------- | ---------------------------------------------------------------------------- |
| A      | 現状維持 (`<agent> <YYYY-MM>` のみ)           | 却下。classify 済みの対象と `normalizeLogs/` に到達できない                  |
| B      | `--normalize` フラグで addOnDir を切り替える  | 却下。閉じた選択子で安全だが、利用者が求めた任意ディレクトリ指定を満たさない |
| **C**  | **`--input-dir` を受理する (他スキルと同形)** | **採用**                                                                     |

### Consequences

**Positive**:

- 他スキルと引数の形が揃い、スキル間で挙動を推測できる
- `normalizeLogs/` を含む任意の対象ツリーへ到達できる

**Negative**:

- DR-07 が塞いだ「未検証データセットへの破壊的書き換え」の経路が開く。
  対象の妥当性は利用者の責任になる
- REQ-C-008 の Rationale が依拠していた「受理範囲＝実測済み範囲」という等式が失われる。
  requirements.md の Q-04 (b) の解決方針を「適用範囲外へ封じる」から
  「対象の明示を求める」へ改める必要がある

---

## DR-39: 対象ディレクトリを再帰的に走査する - 2026-08-21
...
strip の走査は非再帰でした (`findFilesFlat`) 。implementation.md は「対象は
`originalLogs/<agent>/<YYYY-MM>/` 直下に限られる」ことをその根拠としていました。

しかし `classify-chatlogs` は同じディレクトリのログをプロジェクト別サブディレクトリへ移動します。
実測では claude/2026-07 の直下 `.md` は 0 件、配下は 338 件です。
非再帰のままでは、classify 済みの対象に対して strip は常に `total=0` で終わります。

なお specifications.md の孤立退避の定義と R-010 の削除範囲は当初から「対象ディレクトリ **配下**」と
記述されており、再帰化は実装を仕様文へ合わせる方向の変更です。

### Decision

1. **`findFilesFlat` を `findFiles` (再帰・辞書順ソート) へ置き換えます。**
2. **置き換えは本体の列挙・孤立退避の検出 (R-014) ・退避の一括削除 (R-010) の 3 経路に
   同時に適用します。** 部分適用は破壊的な中間状態を作ります。
   - 本体だけ再帰化した場合、R-013 の包含検査がサブディレクトリの `stripped` に対応する退避を
     見つけられず全件不足と判定し、一括削除が恒久的に中止されます
   - 一括削除だけ再帰化した場合、サブディレクトリの孤立退避が未検出のまま `error = 0` となり
     R-011 の保持ゲートを通過し、**復旧材料である退避が削除されます**
3. **R-010 の削除範囲が対象サブツリー全体に及ぶことを受け入れます** (DR-08 決定 2 の範囲拡大) 。

### Consequences

**Positive**:

- classify 済みの対象へ到達できる
- 実装が specifications.md の「配下」という記述と整合する

**Negative**:

- 利用者可視の振る舞い変更。対象ツリーのサブディレクトリに置かれた strip 由来でない `.bak` が一括削除の対象に入る
- **`stripped` が 0 件の実行でも一括削除は走る。** 当該実行に由来する退避が存在しないため、
  サブツリー内のすべての退避が DR-34 の「当該実行に由来しない退避」として `foreign` 警告のうえ
  削除されます。判定規則に一致するファイルが存在しない現状のデータでは、
  通常実行の唯一の観測可能な作用がこの削除になります
- 注入 glob を用いるテストは `findFiles` が内部で発行するディレクトリ列挙パターン
  (`` `${dir}/*/` ``) にも応答する必要がある。パターンを見ずに固定配列を返すモックは
  ファイルをディレクトリとして扱い探索が発散します

---

## DR-40: キャッシュキーが衝突する列挙結果を拒否する - 2026-08-21
...
`ChatlogCache` のキーは拡張子を除くベース名です (`_toHashKey` = `getBasename`) 。
非再帰の走査では 1 ディレクトリ内でファイル名が一意であるため衝突は生じませんでした。

DR-39 の再帰化により、`projA/foo.md` と `projB/foo.md` が同一エントリを共有しえます。
一方が `passthrough` を記録すると、他方は R-003 で `done` と分類され一切検査されません。
`runConcurrent` 下で実行順に依存し、警告も出力されないため無音の取りこぼしになります。

実測では originalLogs / normalizeLogs いずれも重複ベース名は 0 件であり、
現データでは顕在化しません。

### Decision

1. **列挙の直後、判定より前にベース名の重複を検査します。** 重複があれば
   `ChatlogError('FailFast', 'DuplicateBasename', ...)` を送出し、1 件も変更せず終了します
2. **メッセージには衝突したベース名と該当ファイルのパスをすべて含めます。**
3. **`ChatlogCache` のキー生成に注入口を設ける案は採りません。** `_cle-libs` の変更となり
   全スキルのキャッシュに波及します。必要になった時点で別の決定として扱います

### Alternatives Considered

| Option | 内容                                        | 判定                                                            |
| ------ | ------------------------------------------- | --------------------------------------------------------------- |
| A      | 何もしない                                  | 却下。無音で検査から漏れる                                      |
| B      | キャッシュ名前空間を対象ツリーごとに分ける  | 却下。同一実行内の衝突は防げない                                |
| **C**  | **列挙直後に重複を検査し fail-fast する**   | **採用**                                                        |
| D      | `ChatlogCache` にキー生成の注入口を追加する | 保留。長期的には正しいが `_cle-libs` の変更で全スキルへ波及する |
...
## DR-41: 貼り付けチャットログの定型部を Excerpt 前置き区間に限定して除去する - 2026-09-27
...
R-006 の除去範囲は「本文先頭から最初の `## Summary` の直前まで」であり、
発動条件は定型部マーカー `## TOPICS ASSIGNMENT RULES` の行頭完全一致です。

この形は `set-frontmatter` のプロンプト定型部だけを対象としており、
**chatlog-exporter 自身の AI セッションを export したログ**に残る
貼り付けチャットログの定型部には届きません。後者は `## Excerpt` の直後、
すなわち最初の `## Summary` より後ろに位置するためです。

`chatlogs/normalizeLogs/claude/2026/2026-08` (1127 件) の実測:

| 指標                                          | 実測                      |
| --------------------------------------------- | ------------------------- |
| 現行規則での分類                              | 1127 件すべて passthrough |
| 本文に `<<<CHATLOG file="...">>>` を含む      | 736 件                    |
| 本文に `=== <name>.md ===` を含む             | 309 件                    |
| いずれかの貼り付けマーカーを含む (和集合)     | 816 件                    |
| うち最初のマーカーが `## Excerpt` の 3 行以内 | 802 件                    |
| うち最初のマーカーが本文の深い位置にある      | 14 件                     |

後者の 14 件は filter-chatlogs について議論したセッションであり、
区切り文字列を本文中に引用しているだけです。**除去してはなりません。**

貼り付けマーカーの検出自体は既に前例があります。
`constants/patterns/filename.constants.ts` の noise-filter パターンが
「二重日付ファイル名 AND `EXPORTER_PASTE_MARKER_REGEX`」で同種のログを検出しており、
そのコメントは「既知の検出漏れ: 分割済みログ (normalizeLogs) では 11 件が一致しない」と
明記しています。本決定はこの既知ギャップを除去側で塞ぐものです。

### Decision

1. **新規則 R-018 を設けます。** `## Excerpt` の直後から、
   **空行 / 貼り付けマーカー行 / `## 会話ログ` / `### User` / `### Assistant`
   だけが続く連続区間**を取り、その区間内に現れた**最後のマーカー行まで**を除去範囲とします。
   区間はそれ以外の行が現れた時点で打ち切ります
2. **R-006 と R-018 は排他とします。** R-006 が成立したファイルでは R-018 を評価しません。
   1 ファイルから 2 箇所を同時に除去することはしません
3. **貼り付けマーカーは 2 種を対象とします。** `<<<CHATLOG file=` で始まる行
   (先頭の `#` と空白を許容する) と `=== <name>.md ===` の行です。
   いずれも既存定数 (`CHATLOG_DELIMITER_MARK` / `EXPORTER_PASTE_MARKER_REGEX`) を再利用し、
   新たな文字列リテラルを実装ファイルへ直書きしません
4. **Markdown を構文解析しません。** R-005 / R-006 と同じく行頭一致のみで判定します

### Alternatives Considered

| Option | 内容                                                 | 判定                                                       |
| ------ | ---------------------------------------------------- | ---------------------------------------------------------- |
| A      | 何もしない (現状維持)                                | 却下。816 件の貼り付け痕が title / slug / 本文に残り続ける |
| B      | 最初のマーカーから最後のマーカーまでを除去範囲とする | 却下。下記                                                 |
| **C**  | **`## Excerpt` 直後の連続ラッパー区間に限定する**    | **採用**                                                   |
| D      | noise-filter で該当ファイルを削除する                | 却下。本文には実内容が続いており、削除は過剰               |

Option B を却下する理由は誤削除の規模です。マーカーの出現範囲は
357 件が単独行、220 件が 6 行以内である一方、**239 件が 6 行を超えて散在**します。
最後のマーカーまでを範囲にすると、区切り文字列を引用しただけの本文を大量に削ります。
R-007 の安全弁は除去率 99% 超でしか発動せず、50% 規模の誤削除を止められません。

Option C を全 1127 件に当てた実測:
...
区間が実内容に到達しないため、除去量が構造的に有界になります。

### Consequences

**Positive**:

- 貼り付け痕が除去され、後続の set-frontmatter が実内容から title / type / category を判定できる
- 除去量が前置き区間に有界であり、誤削除の上限が設計で決まる
- 深い位置にマーカーを引用しただけの 14 件は passthrough のまま保たれる

**Negative**:

- 判定カスケードに分岐が 1 つ増え、除去範囲が本文先頭以外にも取りうるようになる
- `## Excerpt` を持たない形式のログには効かない (現データでは 816 件すべてが保有)
- ラッパー行の種類を固定値で持つため、exporter 側の出力形式が変わると再検査を要する

---

## DR-42: 書き込み直前の再検証を除去種別ごとのアンカーで行う - 2026-09-27
...
DR-35 が定めた書き込み直前の再検証は、R-008 の除去範囲の形を前提としています。

```text
開始辺: removalStartLine == frontmatter 行数
終了辺: removalEndLine + 1 行目が `## Summary`
```

DR-41 の R-018 はこのいずれも満たしません。除去範囲が `## Excerpt` の次行から始まり、
終了行の次は実内容だからです。再検証をそのまま適用すると、R-018 に該当した全ファイルが
`StaleDecision` となり error に計上され、**1 件も除去されません。**

この不整合は判定側のユニットテストには現れません。判定は正しい範囲を返しており、
破綻するのは書き込み側だからです。結合・システムレベルまで到達して初めて露見します。

再検証そのものを外す案は採れません。行番号による除去は範囲を暗黙にクランプするため、
不整合が例外として現れず、無言の誤削除になります (DR-35) 。

### Decision

1. **`StripDecision` に除去種別 `removalKind` を持たせます。** 値は
   `head` (R-008: 本文先頭〜境界見出しの直前) / `paste` (R-018: Excerpt 前置き区間) /
   `none` (除去範囲を持たない分類) の 3 値とし、定数として定義します
2. **再検証のアンカーを除去種別ごとに切り替えます。**

   | 除去種別 | 開始辺                                         | 終了辺                                    |
   | -------- | ---------------------------------------------- | ----------------------------------------- |
   | `head`   | `removalStartLine` が frontmatter 行数と等しい | `removalEndLine + 1` 行目が `## Summary`  |
   | `paste`  | `removalStartLine - 1` 行目が `## Excerpt`     | `removalEndLine` 行目が貼り付けマーカー行 |

   frontmatter の存在と範囲の順序 (`removalEndLine >= removalStartLine`) は
   除去種別によらず共通の検証とします
3. **`none` に到達した場合は `StaleDecision` として扱います。** 除去範囲を持たない分類が
   書き込み経路へ入ることは設計上ありえませんが、防御的分岐として残します
4. **判定ロジックの再実行による照合は採りません。** 下記

### Alternatives Considered

| Option | 内容                                                     | 判定                                          |
| ------ | -------------------------------------------------------- | --------------------------------------------- |
| A      | 再検証を外す                                             | 却下。無言の誤削除を招く (DR-35 の根拠が有効) |
| **B**  | **除去種別ごとにアンカーを切り替える**                   | **採用**                                      |
| C      | 読み直した内容から除去範囲を再導出し、判定時の範囲と照合 | 却下。下記                                    |

Option C は規則に依存せず厳密ですが、判定ロジックを 1 ファイルにつき 2 度走らせます。
6000 件規模で走査と正規化が倍になり、安全弁の費用として釣り合いません。
またアンカー検査が壊れた場合に判定側と同じ欠陥が両側に現れ、検査が検査になりません。
除去種別ごとのアンカーは判定とは独立した性質を見るため、この共倒れが起きません。

### Consequences

**Positive**:

- R-018 の除去が書き込み側で成立する
- 安全弁の性質 (行番号の暗黙クランプに対する防御) が除去種別によらず維持される
- 検査が判定ロジックから独立しているため、判定側の欠陥を検出しうる

**Negative**:

- 除去規則を追加するたびに再検証のアンカーも追加する必要がある。
  追加を忘れると当該規則は全件 `StaleDecision` になる (error として現れるため無言ではない)
- `StripDecision` にフィールドが 1 つ増える
...
| 2026-09-27 | 3.25.0  | DR-41 を追加 (MINOR: 決定を追加)。R-006 の除去範囲が本文先頭〜最初の `## Summary` 直前に固定されているため、`## Excerpt` 直後に残る貼り付けチャットログの定型部に届かないことを Context として記録 (normalizeLogs/claude/2026/2026-08 の 1127 件すべてが passthrough、貼り付けマーカー保有は 816 件)。新規則 R-018 を設け、`## Excerpt` 直後の連続ラッパー区間 (空行 / 貼り付けマーカー行 / `## 会話ログ` / `### User` / `### Assistant`) 内の最後のマーカー行までを除去範囲とすることを決定。R-006 と R-018 は排他とし 1 ファイルから 2 箇所を同時に除去しない。マーカーは `CHATLOG_DELIMITER_MARK` と `EXPORTER_PASTE_MARKER_REGEX` を再利用する。最初のマーカーから最後のマーカーまでを範囲とする案は、マーカーが 6 行を超えて散在する 239 件で区切り文字列を引用しただけの本文を誤削除し、R-007 の 99% 安全弁では止まらないため却下                                                                |
| 2026-09-27 | 3.26.0  | DR-42 を追加 (MINOR: 決定を追加)。DR-35 の書き込み直前の再検証が R-008 の除去範囲の形 (開始辺 = frontmatter 行数、終了辺の次行 = `## Summary`) を前提としており、DR-41 の R-018 がいずれも満たさず全件 `StaleDecision` となることを Context として記録。`StripDecision` に除去種別 `removalKind` (`head` / `paste` / `none`) を設け、再検証のアンカーを除去種別ごとに切り替えることを決定。frontmatter の存在と範囲の順序は共通検証として維持。再検証を外す案は DR-35 の根拠 (行番号の暗黙クランプによる無言の誤削除) により却下。判定ロジックの再実行による照合は、6000 件規模で走査が倍になること、判定側と同じ欠陥が両側に現れ検査が成立しなくなることにより却下                                                                                                                                                                                                                                    |
