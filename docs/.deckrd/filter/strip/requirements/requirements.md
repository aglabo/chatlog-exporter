...
- `filter strip <agent> <YYYY-MM>` サブコマンドの新設 (入力ディレクトリを指定しない場合 period は必須。REQ-C-008 参照)
...
- Assumptions:
  ...
  - `chatlogs/` 配下へ書き込むのは本ツール群のみであり、strip の実行中に外部プロセス
    ...
    したがって読み取りから書き込みまでの間に対象が差し替わる競合は、検出、および、排他制御を行わない (DR-36)
...
### REQ-F-012: 貼り付けチャットログ定型部の除去

- EARS Type: event-driven

```text
GIVEN 対象ファイルが frontmatter と `## Summary` を持ち、REQ-F-001 の定型部マーカーを持たず、
      `## Excerpt` の直後が空行・貼り付けマーカー行・`## 会話ログ`・`### User`・`### Assistant`
      のみで構成される連続区間で始まり、その区間内に貼り付けマーカー行を 1 つ以上含む
  WHEN `filter strip` が実行される
THEN the system SHALL `## Excerpt` の次行から当該区間内の最後の貼り付けマーカー行までを除去し、
     それ以降を変更せずに保持する。
```
...
| マーカー                                                | 出所                                                       |
| ------------------------------------------------------- | ---------------------------------------------------------- |
| `<<<CHATLOG file=` で始まる行 (先頭の `#` と空白を許容) | filter のバッチプロンプト区切り (`CHATLOG_DELIMITER_MARK`) |
| `=== <name>.md ===`                                     | exporter の貼り付け区切り (`EXPORTER_PASTE_MARKER_REGEX`)  |

連続区間は上記以外の行が現れた時点で打ち切ります。
したがって除去範囲が実内容へ到達することはありません。

REQ-F-001 と本要件は排他です。REQ-F-001 が成立したファイルでは本要件を評価しません。
1 ファイルから 2 箇所を同時に除去することはしません (DR-41 決定 2) 。

**Rationale** (DR-41): chatlog-exporter 自身の AI セッションを export したログには、
貼り付けたチャットログの定型部が `## Excerpt` の直後に残ります。
これは最初の `## Summary` より後ろにあるため REQ-F-001 の除去範囲に入りません。
`chatlogs/normalizeLogs/claude/2026/2026-08` の 1127 件では 816 件が貼り付けマーカーを持ち、
現行規則では 1127 件すべてが passthrough になります。
貼り付け痕は title / slug / 本文を汚染し、後続の set-frontmatter の判定材料を損ないます。

範囲を連続区間に限定する理由は誤削除の抑止です。
最初のマーカーから最後のマーカーまでを範囲とすると、区切り文字列を本文中に引用しただけの
セッション (実測 14 件) を誤って削ります。実測では本要件の除去量は最大 14 行に収まります。

**Acceptance Criteria**:

| AC ID  | Scenario                                                                        |
| ------ | ------------------------------------------------------------------------------- |
| AC-038 | `## Excerpt` 直後の貼り付け前置きが除去され、それ以降が保持される               |
| AC-039 | 貼り付けマーカーが本文の深い位置にしかないファイルが無変更で passthrough となる |
| AC-040 | REQ-F-001 が成立するファイルでは本要件を評価せず、除去箇所が 1 つに保たれる     |
...
対象は単一のディレクトリツリーに定まる (REQ-C-008) ため、
配下の `.bak` はすべて strip の作業対象であり、退避パスを個別に追跡する必要がありません。
REQ-C-009 により走査は再帰的であり、削除範囲は対象サブツリー全体に及びます。
...
安全弁は判定時だけでなく、 **書き込みの直前にも** 置きます (DR-35) 。

除去範囲の確定と実際の書き込みは同一ファイルを別々に読み取るため、
その間にファイルが差し替わると、行番号が古い内容を基準としたまま新しい内容へ適用されます。
行番号による除去は範囲を暗黙にクランプするため、frontmatter や本文が削られても例外になりません。

したがって書き込みの直前に、除去範囲が読み込んだ内容と整合することを検証しなければなりません。
検証する内容は次のとおりです。

- 読み込んだ内容が frontmatter を持つ
- 除去開始行が読み込んだ内容の frontmatter 行数と一致する
- 除去終了行の次の行が境界見出し (`## Summary`) と行頭完全一致する
- 除去終了行が除去開始行以上である
...
| AC-029 | 除去範囲が書き込み時点の内容と整合しないファイルが error として扱われる |
| AC-041 | 除去種別ごとのアンカーで再検証され、R-018 の除去範囲が正しく受理される  |
...
判定が error となったファイル (R-002 / R-007) も、対象パスと成立した規則 ID を添えて
1 件ごとに報告しなければなりません (DR-37) 。
件数のみをサマリーへ計上した場合、6000 件規模の実行ではどのファイルが error になったかを
事後に追う手段が無く、error の解消に着手できません。
規則 ID を添えるのは、R-002 (前提の破れ) と R-007 (安全弁) で原因と対処が全く異なるためです。
`done` を出力しない方針は変更しません。`done` は今回の実行が何もしていない件であり、
error は解消を要する件であって、報告の要否が異なります。
...
| AC-030 | 通常実行で判定 error となったファイルが、対象パスと規則 ID を添えて 1 件ごとに報告される |
...
ただしこの散文の制約が実効性を持たない経路が残ります。入力ディレクトリなしで period を省略した場合は
agent 直下の全年月が処理対象に入るため、REQ-C-008 でこの経路の実行を拒否します。
入力ディレクトリを明示した場合は対象が利用者の指定どおりに定まるため受理しますが、
未検証のデータセットを指すかどうかは利用者の責任になります (DR-38) 。
...
### REQ-C-008: 対象が定まらない実行と出力ディレクトリの指定を拒否する

- EARS Type: unwanted behavior

```text
GIVEN strip の実行が要求された
  WHERE 出力ディレクトリ (`--output-dir` もしくは第 3 位置引数) が指定されている、
        または入力ディレクトリが指定されないまま period が省略されている
  NOT DO 処理対象ファイルを列挙する
THEN the system SHALL 実行を拒否し、ファイルを 1 件も変更してはならない。
```

`strip` は次の 2 つの形を受理します。

- `<agent> <YYYY-MM>` — 対象は `<chatlogsDir>/originalLogs/<agent>/<年>/<YYYY-MM>/`
- 入力ディレクトリ (`--input-dir <path>` または第 1 位置引数のパス) — 対象はそのパス

入力ディレクトリを指定した場合、`resolveChatlogsDir()` の override として働き
agent / period / addOnDir は対象の解決に使われません (他スキルと同じ挙動) 。
したがって period の省略も受理します。period を明示する場合、
不正形式は共通 `parseArgs` が従来どおり拒否します。

**Rationale** (DR-38, DR-07 を上書き): 当初 (DR-07) は、REQ-C-004 の前提が claude/2026-07 の
実測に基づくことを理由に、入力ディレクトリの override も拒否対象としていました。
しかし classify-chatlogs が実施済みの環境では対象がプロジェクト別サブディレクトリへ移動しており、
`<agent> <YYYY-MM>` 形だけでは normalizeLogs をはじめ実運用上の対象へ到達できません。
利用者の決定により override を受理する方針へ改めます。
破壊的書き換えに対する安全側の担保は、退避 (`.bak`) と保持ゲート REQ-F-009 (`error > 0` なら
退避を全保持) が引き続き担います。

出力ディレクトリの拒否は維持します。軸ごとの状況は次のとおりです。
...
| `--input-dir` override | 受理する         | `resolveChatlogsDir()` は override 指定時に agent / period / addOnDir をすべて無視する (`_cle-libs/libs/file-io/resolve-directory.ts`) 。対象は利用者が明示したパスに定まる (DR-38)    |
| 出力ディレクトリの指定 | **破れる**       | 共通 `parseArgs` が `--output-dir` と第 3 位置引数を `outputDir` へ格納するが、strip は in-place で書き換えるため値を参照しない (DR-32)                                                |

したがって本要件が閉じるのは「入力ディレクトリなしでの period の省略」と
「出力ディレクトリの指定」の 2 経路です。
...
| AC ID  | Scenario                                                                                  |
| ------ | ----------------------------------------------------------------------------------------- |
| AC-021 | 入力ディレクトリなしで period を省略して実行すると拒否され、ファイルが 1 件も変更されない |
| AC-022 | `--input-dir` を指定して実行すると受理され、そのディレクトリが対象になる                  |
| AC-027 | 出力ディレクトリを指定して実行すると拒否され、ファイルが 1 件も変更されない               |
| AC-031 | 入力ディレクトリを指定した場合、period を省略しても受理される                             |
| AC-032 | 第 1 位置引数にパスを渡した場合も `--input-dir` と同じ対象に解決される                    |

### REQ-C-009: 対象ディレクトリを再帰的に走査する

- EARS Type: ubiquitous

```text
The system SHALL 対象ディレクトリのサブディレクトリ配下に存在するファイルも、
判定・書き込み・孤立退避の検出・退避の一括削除のすべての対象に含めなければならない。
```

**Rationale** (DR-39): classify-chatlogs はログをプロジェクト別サブディレクトリへ移動します。
非再帰の走査では、classify 実施済みの対象に対して列挙件数が 0 件になり、strip が到達できません。

走査の再帰化は、本体の列挙・孤立退避の検出・退避の一括削除の 3 経路すべてに同時に適用します。
一部だけを再帰化すると、REQ-F-010 の包含検査が全件不足と判定して一括削除が恒久的に中止されるか、
未検出の孤立退避を残したまま REQ-F-009 の保持ゲートを通過して復旧材料の退避が失われます。

**Acceptance Criteria**:

| AC ID  | Scenario                                                             |
| ------ | -------------------------------------------------------------------- |
| AC-033 | サブディレクトリ配下の `.md` が列挙され、判定・書き込みの対象になる  |
| AC-034 | サブディレクトリ配下の孤立退避が `error` として計上される            |
| AC-035 | サブディレクトリ配下の退避が、包含検査を通過したうえで一括削除される |

### REQ-C-010: キャッシュキーが衝突する列挙結果を拒否する

- EARS Type: unwanted behavior

```text
GIVEN 対象ファイルの列挙が完了した
  WHERE 列挙結果に拡張子を除くベース名が同一のファイルが 2 件以上含まれる
  NOT DO 判定・書き込み・キャッシュ記録を行う
THEN the system SHALL 実行を拒否し、衝突したベース名と該当パスをすべて報告しなければならない。
```

**Rationale** (DR-40): キャッシュのキーは拡張子を除くベース名です
(`_cle-libs/classes/ChatlogCache.class.ts` の `_toHashKey`) 。
REQ-C-009 の再帰化により、異なるサブディレクトリの同名ファイルが同一エントリを共有しえます。
一方が `passthrough` を記録すると、他方は判定順序 R-003 で `done` となり一切検査されません。
実行順に依存し警告も出ないため、無音の取りこぼしになります。

**Acceptance Criteria**:

| AC ID  | Scenario                                                                 |
| ------ | ------------------------------------------------------------------------ |
| AC-036 | ベース名が重複する列挙結果では拒否され、ファイルが 1 件も変更されない    |
| AC-037 | 拒否時のメッセージに衝突したベース名と該当ファイルのパスがすべて含まれる |
...
# AC-029: 除去範囲が書き込み時点の内容と整合しないファイルが error として扱われる
# Requirement: REQ-F-008
Scenario: 判定後に内容が差し替わったファイルは書き換えられない
  Given 除去対象と判定されたファイル
  And   判定の確定後に内容が差し替えられ、除去範囲が現在の内容と整合しなくなった状態
  When  除去後の内容を書き込もうとする
  Then  ファイル内容がバイト単位で変更されていない
  And   .bak ファイルが作成されていない
  And   error として計上される
...
# AC-030: 通常実行で判定 error となったファイルが 1 件ごとに報告される
# Requirement: REQ-F-006
Scenario: 判定 error のファイルが事後に特定できる
  Given frontmatter を持たないファイル、除去すると本文が空になるファイル、
        および処理済みのファイルを含むディレクトリ
  When  filter strip を実行する
  Then  frontmatter を持たないファイルと本文が空になるファイルのパスが
        それぞれ 1 行ずつエラーとして出力される
  And   各行に成立した判定規則の ID (R-002 / R-007) が併記される
  And   done と判定されたファイルは出力されない
...
Scenario: 入力ディレクトリなしの period 省略時に実行を拒否する
  Given agent のみを指定し period と入力ディレクトリのいずれも与えない引数
  When  filter strip を実行する
  Then  実行が拒否される
  And   対象ファイルの列挙が行われない
  And   いずれのファイルも変更されない

# AC-022 / AC-031 / AC-032: 入力ディレクトリの指定が受理され、そのディレクトリが対象になる
# Requirement: REQ-C-008
Scenario Outline: 入力ディレクトリの指定を受理する
  Given <指定方法> によって入力ディレクトリを与え period を省略した引数
  When  filter strip を実行する
  Then  実行が受理される
  And   与えた入力ディレクトリが対象として解決される
  And   agent と period は対象の解決に使われない
...
# AC-033 / AC-034 / AC-035: サブディレクトリ配下も走査の対象になる
# Requirement: REQ-C-009
Scenario Outline: サブディレクトリ配下を再帰的に走査する
  Given 対象ディレクトリのサブディレクトリ配下に <対象> が存在する
  When  filter strip を実行する
  Then  当該ファイルが <結果> として扱われる

  Examples:
    | 対象                      | 結果                     |
    | 定型部を持つ .md          | 除去され退避が作られる   |
    | 本体を伴わない .md.bak    | error として計上される   |
    | 当該実行に由来する退避    | 一括削除の対象になる     |

# AC-036 / AC-037: ベース名が重複する列挙結果を拒否する
# Requirement: REQ-C-010
Scenario: キャッシュキーの衝突を検出して実行を拒否する
  Given 拡張子を除くベース名が同一のファイルが 2 件以上含まれる対象ディレクトリ
  When  filter strip を実行する
  Then  実行が拒否される
  And   衝突したベース名と該当ファイルのパスがすべて報告される
  And   いずれのファイルも変更されない
```
...
| Q-04 (b): 他年月の未測定              | **未実測のまま、対象の明示を求めることで解決。** 2026-07 以外の年月は実測していない。入力ディレクトリなしの period 省略時は agent 直下の全年月へ処理が及ぶため REQ-C-008 で拒否する。入力ディレクトリを明示した実行は受理し、未検証データセットを指すか否かは利用者の判断に委ねる (DR-38)           |
...
| REQ-F-006  | AC-008, AC-030                         | Functional     |
| REQ-F-007  | AC-006, AC-009                         | Functional     |
| REQ-F-008  | AC-011, AC-023, AC-029, AC-041         | Functional     |
...
| REQ-F-012  | AC-038, AC-039, AC-040                 | Functional     |
...
| REQ-C-008  | AC-021, AC-022, AC-027, AC-031, AC-032 | Constraint     |
| REQ-C-009  | AC-033, AC-034, AC-035                 | Constraint     |
| REQ-C-010  | AC-036, AC-037                         | Constraint     |
...
| 2026-08-19 | 8.8.0   | DR-37 を反映 (MINOR: AC を追加)。REQ-F-006 に「判定が error となったファイル (R-002 / R-007) も対象パスと成立した規則 ID を添えて 1 件ごとに報告する」を追加し AC-030 を新設。件数のみのサマリーでは 6000 件規模の実行で error のファイルを事後に特定できず解消に着手できないこと、R-002 (前提の破れ) と R-007 (安全弁) で原因も対処も異なるため規則 ID を添えることを根拠として併記。`done` の非出力方針は不変。Traceability の REQ-F-006 を AC-030 へ追随                                                                                                                                                                                                                                                                                                                                                  |
| 2026-08-21 | 9.0.0   | DR-38 / DR-39 / DR-40 を反映 (MAJOR: 決定済みの受理範囲を撤回)。REQ-C-008 から `--input-dir` override の拒否を撤回し、入力ディレクトリ (`--input-dir` または第 1 位置引数のパス) を受理する形へ改訂。指定時は agent / period を対象の解決に用いないため period 省略も受理する。出力ディレクトリの拒否は維持。AC-022 を受理側へ反転し AC-031 / AC-032 を追加。REQ-C-009 (対象ディレクトリの再帰走査。本体列挙・孤立退避の検出・退避の一括削除の 3 経路に同時適用) と AC-033 / AC-034 / AC-035 を新設。REQ-C-010 (キャッシュキーが衝突する列挙結果の拒否) と AC-036 / AC-037 を新設。Q-04 (b) の解決方針と §5 の散文制約を追随                                                                                                                                                                                 |
| 2026-09-27 | 9.1.0   | DR-41 を反映 (MINOR: 要件を追加)。REQ-F-012 (貼り付けチャットログ定型部の除去) と AC-038 / AC-039 / AC-040 を新設。`## Excerpt` の次行から、空行・貼り付けマーカー行・`## 会話ログ`・`### User`・`### Assistant` のみが続く連続区間内の最後のマーカー行までを除去範囲と規定。マーカーは `CHATLOG_DELIMITER_MARK` 由来の `<<<CHATLOG file=` 行 (先頭の `#` と空白を許容) と `EXPORTER_PASTE_MARKER_REGEX` の `=== <name>.md ===` 行の 2 種。REQ-F-001 とは排他とし 1 ファイルから 2 箇所を同時に除去しない。normalizeLogs/claude/2026/2026-08 の 1127 件中 816 件が貼り付けマーカーを持ちながら現行規則では全件 passthrough になること、範囲を連続区間に限定しないと引用のみの 14 件を誤削除することを Rationale に併記。Traceability に REQ-F-012 を追加。REQ-F-001〜REQ-F-011 / REQ-C-001〜REQ-C-010 は不変 |
| 2026-09-27 | 9.2.0   | DR-42 を反映 (MINOR: AC を追加)。REQ-F-008 の安全弁に AC-041 を追加。書き込み直前の再検証 (AC-029) が R-008 の除去範囲の形を前提としており、R-018 の除去範囲では開始辺・終了辺がいずれも不成立となって全件 error になることを根拠として併記。再検証を除去種別 (`head` / `paste` / `none`) ごとのアンカーへ一般化し、R-018 の除去範囲が正しく受理されることを検証可能にした。Traceability の REQ-F-008 を AC-041 へ追随。REQ-F-012 と R-018 の内容は不変                                                                                                                                                                                                                                                                                                                                                      |
