...
based-on: specifications.md v6.2.0
status: Draft
version: 3.9.0
created: "2026-08-13"
---
...
実行フェーズ(Phase 0〜7)と判定規則(R-001〜R-018)の対応。全 18 規則がいずれか 1 つの
...
| R-006 | 2            | 除去範囲の確定 (不成立時は R-018 へ)           | 7           |
| R-018 | 2            | 除去範囲の確定 / passthrough (+ 記録。DR-31)   | 7 / 11      |
| R-007 | 2            | error                                          | 7           |
...
| 2026-09-27 | 3.8.0   | DR-41 を反映 (MINOR: 実装対象を追加)。Rule Coverage 表に R-018 の行を追加し、R-006 の分類を 「passthrough (+ 記録)」から「除去範囲の確定 (不成立時は R-018 へ)」へ書き換え。規則数を 17 から 18 へ是正。R-018 は `libs/strip-boundary.ts` に `findPasteWrapperRange` を追加し `classifyStrip` (Commit 7) から呼ぶ。`passthrough` の記録 (Commit 11 の `_recordPassthrough`) は R-006 と R-018 がいずれも不成立の場合に成立規則として記録する。定数は `constants/strip.constants.ts` へ置き、貼り付けマーカーは `CHATLOG_DELIMITER_MARK` と `EXPORTER_PASTE_MARKER_REGEX` を再利用する。based-on を specifications.md v6.1.0 へ更新。Commit 分割・実行フェーズ・既存規則の評価順序は不変                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 2026-09-27 | 3.9.0   | DR-42 を反映 (MINOR: 実装対象を追加)。`writeStripped` (Commit 8) の書き込み直前の再検証を、`StripDecision.removalKind` (`head` / `paste` / `none`) による分岐へ改める。`head` は従来のアンカー (frontmatter 行数 / `## Summary`)、`paste` は `removalStartLine - 1` 行目が `## Excerpt`・`removalEndLine` 行目が貼り付けマーカー行、`none` は不整合として `StaleDecision`。貼り付けマーカー行の判定は `libs/strip-boundary.ts` の述語を共有し、`write-stripped.ts` へ literal を直書きしない。`removalKind` の定数と派生型は `types/strip-removal-kind.const.types.ts` に置く (`STRIP_CACHE_STATUSES` と同じ `as const` + 派生 union の形)。based-on を specifications.md v6.2.0 へ更新                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
