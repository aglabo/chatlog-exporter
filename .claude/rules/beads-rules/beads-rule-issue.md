...
トークナイザは `#` コメントを認識し、**コメント本文はコマンドとして解釈しない。**
そのため起票例をコメントに書いてもブロックされない。

```bash
# 通る。どちらも起票していない
echo ok # bd create --title x
# bd create --title x
```

コメントとみなすのは**語の開始位置にある `#` だけ**。入力先頭・空白・タブ・改行・`;`・`&`・`|`
の直後がこれに当たる。次の形は `#` をリテラルとして扱い、通常どおり検査する。

- 語中の `#`（`16#ff`・`${var#pat}`・`a#b`・URL のフラグメント）
- 引用符の中の `#`（`"#"`・`'#'`）
- `\` + 改行で継ぎ足した語の途中（`echo x\` 改行 `#foo` は bash では 1 語 `x#foo`）

**コメント判定は heredoc 認識より前に行う。** コメント内の `<<` は heredoc として積まないため、
`echo ok # <<true` のような行で本文側の `bd create` を素通しさせることはできない。
同様に、コメントに `BEADS_NO_PARENT=1` を書いても免除にはならない。
