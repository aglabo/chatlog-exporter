...
```bash
# it ラベルに割り当てられた ID を抽出し、2 ファイル以上に出現するものを報告する
for f in $(grep -rl "it(" --include=*.spec.ts .); do
  grep -ohE "it\((['\"\`])[^'\"\`]*" "$f" \
    | tr -c 'A-Za-z0-9-' '\n' \
    | grep -xE "T-[A-Z0-9]+(-[A-Z0-9]+)*-[0-9]{2}(-[0-9]{2})?" \
    | sort -u | sed "s|\$|\t$f|"
done | sort | cut -f1 | uniq -d
```

出力が空であること。以下の 2 点を外すと誤検出する。

- `it(...)` ラベル内に限定しないと、JSDoc の相互参照を割り当てと誤認する
